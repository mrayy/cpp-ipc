...
class robust_mutex : public sync::obj_impl<a0_mtx_t> {
 public:
  bool lock(std::uint64_t tm) noexcept {
    ...
    for (;;) {
      ...
      switch (eno) {
        ...
        default:
          ipc::error("fail mutex lock[%d]:%s\n", eno, a0_strerror((a0_err_t)eno));
          return false;
      }
    }
  }
  ...
  bool unlock() noexcept {
    ...
    if (eno != 0) {
      ipc::error("fail mutex unlock[%d]:%s\n", eno, a0_strerror((a0_err_t)eno));
      return false;
    }
    return true;
  }
...
class mutex {
  ...
  void acquire_mutex(char const *name) {
    ...
    if (it == info.mutex_handles.end()) {
      it = info.mutex_handles.emplace(name, curr_prog::shm_data::init{name}).first;
    }
 ...
 public:
  ...
  a0_mtx_t const *native() const noexcept { return valid() ? mutex_->native() : nullptr; }

  a0_mtx_t *native() noexcept { return valid() ? mutex_->native() : nullptr; }

  bool valid() const noexcept { return (mutex_ != nullptr) && (ref_ != nullptr) && mutex_->valid(); }
  ...
  void close() noexcept {
    if ((mutex_ != nullptr) && (ref_ != nullptr)) {
      if (mutex_->name() != nullptr) {
        release_mutex(mutex_->name(), [this] { return ref_->fetch_sub(1, std::memory_order_relaxed) <= 1; });
      } else
        mutex_->close();
    }
    mutex_ = nullptr;
